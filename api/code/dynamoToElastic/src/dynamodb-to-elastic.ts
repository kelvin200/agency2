...
export const dynamoDBToElastic = (): HandlerPlugin<ElasticsearchContext> => ({
  type: 'handler',
  async handle(context) {
    ...
    for (const ops of chunk(operations, 100)) {
      try {
        const res = await context.elasticsearch.bulk({ body: ops })
        checkErrors(res)
        if (process.env.DEBUG === 'true') {
          console.log('Bulk response', JSON.stringify(res, null, 2))
        }
        // Let ES server breath
        delay(200)
      } catch (error) {
        if (process.env.DEBUG === 'true') {
