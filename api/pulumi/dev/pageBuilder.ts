...
class PageBuilder {
  ...
  constructor({
    ...
    const pbInstallationZipPath = path.join(
      path.resolve(),
      '.tmp',
      'pbInstallation.zip',
    )
    ...
    const policy =
      policies.getPbUpdateSettingsLambdaPolicy(primaryDynamodbTable)

    new aws.iam.RolePolicyAttachment(
      `pb-update-settings-lambda-role-policy-attachment`,
      {
        role: this.role,
    ...
    new aws.iam.RolePolicyAttachment(
      `pb-update-settings-lambda-AWSLambdaBasicExecutionRole`,
      {
        role: this.role,
    ...
    const updateSettings = new aws.lambda.Function('pb-update-settings', {
      ...
      code: new pulumi.asset.AssetArchive({
        '.': new pulumi.asset.FileArchive(
          '../code/pageBuilder/updateSettings/build',
        ),
      }),
      environment: {
        variables: {
    ...
    this.exportPagesLambdaRole = new aws.iam.Role(
      'pb-export-pages-lambda-role',
      {
        assumeRolePolicy: {
          Version: '2012-10-17',
          Statement: [
    ...
    new aws.iam.RolePolicyAttachment(
      `pb-export-pages-lambda-role-policy-attachment`,
      {
    ...
    new aws.iam.RolePolicyAttachment(
      `pb-export-pages-lambda-AWSLambdaBasicExecutionRole`,
      {
        role: this.exportPagesLambdaRole,
        policyArn: aws.iam.ManagedPolicy.AWSLambdaBasicExecutionRole,
      },
    ...
    const exportPagesCombineLambda = new aws.lambda.Function(
      'pb-export-pages-combine',
      {
        ...
        code: new pulumi.asset.AssetArchive({
          '.': new pulumi.asset.FileArchive(
            '../code/pageBuilder/exportPages/combine/build',
          ),
        }),
        environment: {
          variables: {
    ...
    const exportPagesProcessLambda = new aws.lambda.Function(
      'pb-export-pages-process',
      {
        role: this.exportPagesLambdaRole.arn,
        runtime: 'nodejs14.x',
        handler: 'handler.handler',
        ...
        code: new pulumi.asset.AssetArchive({
          '.': new pulumi.asset.FileArchive(
            '../code/pageBuilder/exportPages/process/build',
          ),
        }),
        environment: {
          variables: {
    ...
    this.importPagesLambdaRole = new aws.iam.Role(
      'pb-import-page-lambda-role',
      {
        assumeRolePolicy: {
          Version: '2012-10-17',
          Statement: [
    ...
    new aws.iam.RolePolicyAttachment(
      `pb-import-page-lambda-role-policy-attachment`,
      {
    ...
    new aws.iam.RolePolicyAttachment(
      `pb-import-page-lambda-AWSLambdaBasicExecutionRole`,
      {
        role: this.importPagesLambdaRole,
        policyArn: aws.iam.ManagedPolicy.AWSLambdaBasicExecutionRole,
      },
    ...
    const importPagesQueueProcess = new aws.lambda.Function(
      'pb-import-page-queue-process',
      {
        ...
        code: new pulumi.asset.AssetArchive({
          '.': new pulumi.asset.FileArchive(
            '../code/pageBuilder/importPages/process/build',
          ),
        }),
        environment: {
          variables: {
    ...
    const importPagesQueueCreate = new aws.lambda.Function(
      'pb-import-page-queue-create',
      {
        role: this.importPagesLambdaRole.arn,
        runtime: 'nodejs14.x',
        handler: 'handler.handler',
        ...
        code: new pulumi.asset.AssetArchive({
          '.': new pulumi.asset.FileArchive(
            '../code/pageBuilder/importPages/create/build',
          ),
        }),
        environment: {
          variables: {
