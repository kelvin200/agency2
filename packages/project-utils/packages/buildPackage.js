...
module.exports = async options => {
  ...
  options.logs !== false &&
    console.log(`Done! Build finished in ${duration + 's'}.`)
...
const babelCompile = async ({ cwd }) => {
  ...
  const files = glob.sync(join(cwd, 'src/**/*.*').replace(/\\/g, '/'), {
    nodir: true,
  })
  const compilations = []
  const copies = []
...
const tsCompile = ({ cwd, overrides, debug }) => {
  return new Promise((resolve, reject) => {
    ...
    const { projectReferences, options, fileNames, errors } =
      parsedJsonConfigFile
